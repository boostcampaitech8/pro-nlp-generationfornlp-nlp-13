...
def main():
    ...
    wandb_config = {
        ...
        "inferential_model": model_cfg['model']['inferential']['model_name'],
        
        # LoRA 하이퍼파라미터 (lora_params에서 추출)
        "lora_r": model_cfg['lora']['inferential']['r'],
        "lora_alpha": model_cfg['lora']['inferential']['lora_alpha'],
        
        # 학습 설정 (train_cfg에서 추출)
        "inferential_lr": train_cfg['training']['inferential'].get('learning_rate'),
        "inferential_batch_size": train_cfg['training']['inferential'].get('per_device_train_batch_size'),
        "inferential_epochs": train_cfg['training']['inferential'].get('num_train_epochs'),
