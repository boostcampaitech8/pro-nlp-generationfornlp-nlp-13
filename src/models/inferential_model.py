...
class InferentialModel:
    """
    추론형(5지선다) 문제를 해결하기 위해 양자화된 LoRA 모델과 토크나이저를 관리하는 클래스.
    """
    
    def __init__(self, checkpoint_path: str, device_map="auto"):
        """
        지정된 체크포인트로부터 8비트 양자화된 모델과 토크나이저를 로드합니다.
        
        Args:
            checkpoint_path: PEFT 체크포인트가 저장된 경로
            device_map: 모델 파라미터를 장치에 할당하는 전략 (기본값: "auto")
        """
        print(f"[InferentialModel] Loading from {checkpoint_path}...")
