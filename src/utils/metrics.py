...
class Metrics:
    def __init__(
        ...
        self.label_pos_from_tail =  label_pos_from_tail
        self.logit_pos_from_tail = logit_pos_from_tail

        self.digit_ids = []
        self._make_digit_ids()
    ...
    def compute_metrics(
        self, 
        eval_pred: Union[Tuple[np.ndarray, np.ndarray], Any]
    ) -> Dict[str, float]:
        """
        eval_pred:
        ...
        pos = (real_len - self.label_pos_from_tail).clamp(min=0, max=L - 1)  # (B,)
        ...
        gold_cls = gold_tok - self.digit_ids[0]
    ...
    def preprocess_logits_for_metrics(
        self, 
        logits: Union[torch.Tensor, Tuple], 
        labels: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        # digit ids만 슬라이스 -> (B, 5)
        picked_digits = picked[:, self.digit_ids]
